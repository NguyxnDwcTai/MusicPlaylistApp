...
export default function Sidebar({ isMobileMenuOpen, setIsMobileMenuOpen }: SidebarProps) {
  ...
  const actionItems: { name: string; path: string; icon: any; iconColor?: string; action?: () => void }[] = [
  ...
  return (
    <>
      ...
      <aside className={`fixed md:static inset-y-0 left-0 z-50 w-64 bg-paper-2 border-r border-rule flex flex-col h-full text-ink select-none transform transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
        }`}>
        {/* App Branding */}
        <div className="p-6">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-4 px-4 py-3 rounded-lg text-[13.5px] font-medium transition-all duration-200 group ${active
              ...
              >
                <Icon
                  size={18}
                  className={`transition-transform duration-200 group-hover:scale-105 ${active ? 'text-accent' : 'text-neutral group-hover:text-ink'
                    }`}
                />
                {item.name}
              </Link>
            ...
            const content = (
              <span className="flex items-center gap-4 px-4 py-3 rounded-lg text-[13.5px] font-medium transition-all duration-200 group cursor-pointer w-full text-left">
                <span className={`p-1 rounded-sm bg-paper-3 group-hover:bg-paper-2 border border-rule transition-colors`}>
                  <Icon
                    size={14}
                    className={`transition-transform duration-200 group-hover:scale-105 ${item.iconColor || 'text-neutral group-hover:text-ink'
                      }`}
                  />
                </span>
            ...
            playlists.map((playlist) => {
              ...
              return (
                <Link
                  ...
                  onClick={handleLinkClick}
                  className={`block px-4 py-2 rounded-lg text-[13px] truncate transition-colors ${active
